...
export class CarService extends ResourceService<any> {
  ...
  getLastYearVehicles(): Observable<any> {
    return this.http
      .get<any>(`${this.basePath}/latest/vehicles`, this.httpOptions)
      .pipe(retry(2), catchError(this.handleError));
  }
  getVehiclesByMostStars(): Observable<any> {
    return this.http
      .get<any>(`${this.basePath}/stars/vehicles`, this.httpOptions)
      .pipe(retry(2), catchError(this.handleError));
  }

  get20VehicleRandom(): Observable<any> {
    return this.http
      .get<any>(`${this.basePath}/random/vehicles`, this.httpOptions)
      .pipe(retry(2), catchError(this.handleError));
  }

  getVehicleByOwner(): Observable<any> {
    return this.http
      .get<any>(`${this.basePath}/by/owner`, this.httpOptions)
      .pipe(retry(2), catchError(this.handleError));
  }

  getVehicleInfo(): Observable<any> {
    return this.http
      .get<any>(`${this.basePath}/humidity/temperature`, this.httpOptions)
      .pipe(retry(2), catchError(this.handleError));
  }
}
