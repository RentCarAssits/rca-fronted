...
export class ProposalListComponent {
  ...
  getProposal(userId: number) {
    this.proposalService.getAll().subscribe(response => {
  ...
  seeServiceItem(proposalId: number) {
    let width = '60%';
    const windowWidth =
      window.innerWidth ||
    ...
    this.dialogService.open(ServiceItemListComponent, {
      header: 'Service Item List',
      width: width,
  ...
  accept(proposalId: number) {
    this.proposalService.accept(proposalId).subscribe(response => {
