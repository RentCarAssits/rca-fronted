...
@Component({
  selector: 'app-request-item-list',
  templateUrl: './request-item-list.component.html',
  styleUrls: ['./request-item-list.component.css']
})
export class RequestItemListComponent {
  user:any;
  requestItems!: any;
  diagnosticAux!: any
  selectedItem: any;
  ...
  constructor(private http: HttpClient,private authService:AuthService,
    ...
    this.serviceItemId = this.config.data.serviceItemId
  }
  ngOnInit(): void {
    this.user = this.authService.getCurrentUser();
    console.log("user as",this.user)
    this.getProposal(this.proposalId, this.serviceItemId);

  }
  getProposal(proposalId: number, serviceItemId: number) {
    this.serviceItemService.getAllRequestItems(proposalId,serviceItemId).subscribe(response => {
      this.requestItems = response.result
